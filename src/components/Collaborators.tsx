const benefits = [
  {
    icon: (
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#0096C7" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-12">
          ...
          <h2 className="font-display font-bold text-4xl md:text-5xl text-ocean-900 mb-4">
            ¿Tu organización quiere impulsar<br />
            <span className="gradient-text">el tech de la costa?</span>
          </h2>
          <p className="text-slate-500 text-lg max-w-xl mx-auto">
            Estamos buscando instituciones, empresas y comunidades que quieran colaborar y
            co-crear el ecosistema tech de Mar del Plata.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {benefits.map((b) => (
            <div
              key={b.title}
              className="bg-slate-50 border border-slate-100 rounded-2xl p-7 text-center transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-ocean-600/10"
            >
              <div className="w-14 h-14 bg-ocean-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
                {b.icon}
              </div>
              <h3 className="font-display font-bold text-lg text-ocean-900 mb-2">{b.title}</h3>
