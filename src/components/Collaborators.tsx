        ...
        <div className="overflow-x-auto pb-2">
          <div className="flex gap-4 snap-x snap-mandatory">
            {members.map((member) => (
              <article
                key={member.id}
                className="min-w-[260px] md:min-w-[300px] snap-start bg-slate-50 border border-slate-100 rounded-2xl p-5"
              >
                <div className="flex items-center gap-4 mb-4">
                  ...
                  />
                  <div>
                    <h3 className="font-display font-bold text-lg text-ocean-900 leading-tight">
                      {member.full_name || "Miembro"}
                    </h3>
                    <p className="text-slate-500 text-sm">{member.bio || "Miembro de la comunidad"}</p>
                  </div>
                </div>
                {(member.github_url || member.linkedin_url || member.twitter_url) && (
                  <div className="flex items-center gap-3">
                    {member.github_url && (
                      <a href={member.github_url} target="_blank" rel="noopener noreferrer" className="text-slate-500 hover:text-ocean-700 transition-colors" aria-label="GitHub">
                    ...
                    {member.linkedin_url && (
                      <a href={member.linkedin_url} target="_blank" rel="noopener noreferrer" className="text-slate-500 hover:text-ocean-700 transition-colors" aria-label="LinkedIn">
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                    ...
                    {member.twitter_url && (
                      <a href={member.twitter_url} target="_blank" rel="noopener noreferrer" className="text-slate-500 hover:text-ocean-700 transition-colors" aria-label="X">
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
